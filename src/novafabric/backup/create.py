...
def is_denied(rel_path: PurePosixPath | str) -> bool:
    ...
    *,
    ...
    if profile in ("pg", "postgres", PROFILE_PG_DUMP):
        ...
        raise BackupCreateError(
            f"Unknown backup profile {profile!r} — expected 'local' or 'pg'"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="nova-backup-") as tmp:
        ...
        if registry_db.exists():
            ...
            members.append(_member(snapshot, "registry.db", kind="registry"))
            if db_dump_ref is None:
                db_dump_ref = "registry.db"

        # --- capsule directories ---
        for dirname in _CAPSULE_DIRS:
            ...
            for src in sorted(p for p in src_dir.rglob("*") if p.is_file()):
                ...
                members.append(_member(dst, str(rel), kind="blob"))
        ...
        if config_src.is_file():
            ...
            members.append(_member(redacted_path, redacted_config, kind="config"))
        ...
        _write_archive(archive_path, manifest_path, dsse_path, members, stage)
...
def _sqlite_snapshot(src_db: Path, dst_db: Path) -> None:
    ...
    try:
        ...
        raise BackupCreateError(f"SQLite online backup of {src_db} failed: {exc}") from exc
    finally:
        src.close()
...
def _member(file_path: Path, arc_path: str, *, kind: str) -> BackupMember:
