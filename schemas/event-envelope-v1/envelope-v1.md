...
| `event_type` | `string` enum | Yes | Event kind. Real producers emit `RunStarted`/`RunCompleted`/`RunFailed`/`RunAborted` at run boundaries, plus `ModelCallCompleted`/`ModelCallFailed`/`ToolCallCompleted`/`ToolCallFailed` re-emitted per locally-captured call (matching `schemas/event_schema.py::CapsuleEventType` — ADR-0220; no `*Started` variant for calls, since the source JSONL has one record per completed/failed call, never a separate start event). The original `run.start`, `run.end`, `model_call`, `tool_call`, `span`, `capsule.finalize` values remain schema-valid for backward compatibility but were never actually emitted by any producer in this repository. Indexed by the metadata database. |
