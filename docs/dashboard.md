...
The dashboard ships **25 tabs**, grouped by workflow. (Source of truth:
...
| Commands | CLI | v0.8 | 35+ CLI command builders with live preview |
...
| CLI command reference with live preview | *(all `nova` sub-commands)* | ✅ Commands tab | **v0.8:** 13 command builders for core capture, registry, and replay. **v0.12.6:** expanded to 35 builders across 4 journey tracks: *Debug & Replay* (inspect, validate, forensic/semantic/exact replay, diff, rollback, run-show), *Govern & Approve* (register, promote, suggest-register, unregister, export-evidence), *Audit & Lineage* (lineage provenance/blast-radius/replay-chain/time-travel, audit, holds), *Infrastructure & Scaling* (serve, server, api-proxy, mcp-proxy, capture with all runners, collector). Each builder renders a live command preview as you fill in the form, with a copy button. |
