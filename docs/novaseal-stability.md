...
| `NovaSeal.verify(capsule_dir)` → `VerifyResult` | **Stable** | No breaking changes without `NOVASEAL_MAJOR` increment |
...
| `KeyConfig`, `SealConfig` Pydantic models | **Stable** | Additive field additions OK; removals require `NOVASEAL_MAJOR` |
| `load_signing_profile()` return type | **Stable** | |
| `VerifyResult` fields | **Stable** | Additive only until `NOVASEAL_MAJOR` |
...
NOVASEAL_MAJOR = 1  # increment on any breaking cryptographic API change
```
...
5. **`VerifyResult` field removal or type change** that could cause a verifier to
...
- Adding optional fields to `SealBundle`, `VerifyResult`, or `ToolPermissionEvent`.
- Adding new `VerifyResult` check flags (additive).
...
4. Add a `seal_format_version` field to `SealBundle` and `VerifyResult` to allow
   format detection.
...
- Adding new `VerifyResult` check fields with `default=False`.
