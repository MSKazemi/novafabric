...
| Symptom | Diagnosis | Action |
|---|---|---|
| Everything returns 401 after upgrading to ≥ v0.61 (local mode) | Breaking default: with OIDC off, the server now requires the local bearer token | Read the token from `~/.novafabric/.server-token` (`$HOME`/`$NOVAFABRIC_HOME`, mode 0600). It is printed at startup **only when stderr is an interactive terminal** (ADR-0263): under systemd, Docker/Kubernetes, `nohup`, or CI the start-up line shows a non-reversible *fingerprint* plus the file path instead, and the application logger has never carried the secret. So do **not** expect to find the token in journald or an aggregated log sink — read the file, or pin `NOVAFABRIC_SERVER_TOKEN`. Pin a stable one via `NOVAFABRIC_SERVER_TOKEN` for CI/Docker |
