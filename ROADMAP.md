...
> **Label glossary:** `experimental` — ships and works, interface may change before v1.0 schema freeze; `prototype` — implemented but not validated at target scale or runtime; `planned` — not yet implemented; `research` — lives only in the maintainers' private `design/research/`. No item is listed as shipped until tests pass and the feature ships in a release.
>
...
## Now / Next / Later — the 10-second version

851 lines is a reference, not an orientation. This is where the project actually is,
and where an outside contributor fits.
...
Real-time monitoring and alerting · a hosted SaaS · a prompt playground.
[Why](docs/comparison.md#where-novafabric-is-genuinely-the-wrong-choice).
...
> tagged. The table below is current through **v0.101.0**.

| Version | Feature | Status |
|---|---|---|
| v0.101.0 | **Enterprise-grade program — ten first slices (ADRs 0240–0249).** Durable background jobs (lease-claimed store + runner; `/v0/replays` state survives restarts and `--workers`); native listener TLS on both servers (fail-closed validation, real-HTTPS-tested); the split-brain-proof writer lease with fencing tokens (Postgres + SQLite twins, live-container-verified); per-tenant KEKs with tenant-scoped revocation; jurisdiction-aware capsule routing (fail-closed placement + the first consumer of the ADR-0077 residency gate); step-up fresh-auth on destructive actions + role mutations joining the hash-chained audit log; the serve/ OpenAPI contract with a shrink-only annotation ratchet (198 routes inventoried); the SLO catalog (gates read thresholds from the published source; measured numbers expire visibly); `docs/support-policy.md` + every CI gate named in the documented release process; and the signed, offline-verifiable air-gap bundle. All additive, opt-in, and labelled; each ADR records its deferred slices. Gate: 11,864 passed / 0 failed, coverage 92.85%. Also in this release, outside the ten-ADR program: the **persistent `nova query` index** (ADR-0225, [#6](https://github.com/MSKazemi/novafabric/issues/6)) — an mtime-validated SQLite cache outside the capsule directory, with the indexer signature coupling that makes a stale cache impossible rather than merely unlikely. See [docs/releases/v0.101.0.md](docs/releases/v0.101.0.md). | **experimental** |
| v0.100.1 | **Release-tag workflow fixes; no library changes.** The container image had been unbuildable since v0.99.0 — `force-include` entries for the canonical JSON Schemas landed with no matching `COPY` in the Dockerfile, the *second* instance of that omission. Invisible locally, because `uv build` on a dev machine has the repo root as its build context; only Docker's narrower context exposes it. Guarded now. Also: the MetadataStore Security Gate installed without `--all-extras` (fifth instance of the BL-022 class — the guard now scans every workflow rather than naming jobs), and `publish-image.yml`'s manual-retry `workflow_dispatch` derived no image tag from a branch, so the escape hatch failed in exactly the case it exists for. See [docs/releases/v0.100.1.md](docs/releases/v0.100.1.md). | **experimental** |
| v0.100.0 | **The release pipeline, CI, and the contributor surface.** v0.98.0–v0.99.0 were tagged but **never reached PyPI** — a blocking SBOM step took the whole build job down for five consecutive releases, so three fixes for released installs never reached a user. Fixed, plus three HIGH-severity CVEs cleared (`cryptography` 48→50, `aiohttp` 3.14.1→3.14.3; the major crypto bump verified against 621 crypto-path tests and the p99 seal gate). CI had been failing three ways: `integration` had run no test since 2026-07-30 (missing `--all-extras`), `web` could not build (Node 20 vs Astro 7's ≥22.12), and `unit` had **never completed** — 30 runs hitting a 15-minute cap that GitHub reports as `cancelled`, so the suite and the 90% coverage gate had not run in CI at all. Postgres migrations could not run as documented (`psycopg2` vs psycopg 3). On the community side: **151 dead public-doc links** fixed and guarded by `make check-links` (the rule: a target must be *tracked by the public git*, not merely exist), public governance docs + RFC process, a 225-entry decisions index, a public architecture map, an honest comparison page, a 15-minute contributor path, devcontainer, CODEOWNERS, issue forms, published response SLAs, and `docs/` published as 48 web pages. See [docs/releases/v0.100.0.md](docs/releases/v0.100.0.md). | **experimental** |
| v0.99.0 | **Packaging hardening + four correctness fixes that were live on released installs.** ADR-0222 moves `duckdb`/`pyarrow`/`python-louvain`/`clickhouse-connect` out of core into extras: a plain `pip install novafabric` drops **412 MB/50 packages → 113 MB/42**, with a degradation contract at every core-reachable call site. Then four defects that a source-tree test run could not see: **three schema validators (Event Envelope v1, batch-import manifest, parent/child capsule) raised `FileNotFoundError` on every `pip install`** because their schemas were never in the wheel — now `force-include`d with packaged-path-first loaders; **~1.5% of API keys could never be rotated or revoked** (`token_urlsafe` gives 1 in 65 ids a leading `-`, which Click parsed as an option); **concurrent in-process captures filed one run's events into another run's capsule** (8 of 9 call sites drove the process-global hooks unguarded — ADR-0224 adds an ownership token, and all 8 adapters now record `metadata.wire_capture`); and **nine schema `$id` collisions** where one identity resolved to two contradicting documents (ADR-0223 — the canonical OAS v1 *targets* are re-identified `-v1.schema.json`; producers are unchanged because ADR-0034 §1 keeps the spec in pre-freeze draft). Plus **ADR-0024's license policy is enforced in CI for the first time** (`scripts/license_gate.py` + `.license-policy.toml`, disjunction-aware, Tier D forbidden outright), the `nova query` DuckDB index build is ~880× faster (columnar Arrow path), and `scripts/` is inside the ruff gate. ADR-0225 records the persistent-query-index design (13× headroom measured); implementation deferred. See CHANGELOG `[0.99.0]`. | **experimental** |
| v0.98.1 | **Documentation audit + a published-claim correction** — full sweep of every doc surface against what v0.97/v0.98 actually shipped. Corrects v0.98.0's deploy-path overclaim (server mode is opt-in via `NOVA_MODE=server`; the **default is still the `--insecure` dashboard**, so that audit finding is advanced-not-closed), an operator-guide section that described SAML as unimplemented when its gate closed in v0.73.0, a false "dashboard is read-only" claim, and developer-guide instructions that predate the tab decomposition. Adds the navigation documentation that never existed (7 groups, `?tab=`/`?sub=` deep links, `g`-key shortcuts), deployment-mode env vars, the new CLI flags, dashboard test tiers, and architecture coverage of the `web/` front end. Version drift fixed in README/CITATION.cff/api-reference/architecture README. See CHANGELOG `[0.98.1]`. Docs only — no code change. | **experimental** |
| v0.98.0 | **Enterprise-readiness + trust surfaces** — server-mode app factory + `nova server start --workers N` (closes the audit's structurally-impossible-workers finding), opt-in Postgres connection pool (ADR-0221) wired through the factory with live `nova_db_pool_*` gauges, request-ID correlation + `--log-format json`, cosign/SBOM/SLSA attestation for published images and wheels, and a container/Helm path that can run `nova server start` with real `httpGet /livez`+`/readyz` probes (opt-in via `NOVA_MODE=server`; the default is still the `--insecure` dashboard, so that audit finding is only partly closed). Six P0 security fixes (bearer token off the logger, CSPRNG audit sampling, fail-closed legal holds, webhook SSRF guard, replay read auth). **Trust-surface views (ADR-0173/0174)** — SVG radar glyph + redaction heat overlay in the Seal tab on the already-shipped endpoints; an unverifiable guarantee renders n/a and is excluded from the claim polygon. Opt-in orphan pruning for the runs index. Dashboard-architecture ADRs re-statused with their deviations recorded. See CHANGELOG `[0.98.0]` + [docs/releases/v0.98.0.md](docs/releases/v0.98.0.md). | **experimental** |
| v0.97.0 | **Dashboard modernization program** — design-system layer (`web/src/components/ui/primitives/`: 14 primitives, tokens v2, lucide icons, 10px type floor), balanced 7-group navigation with stable `g`-key shortcuts (all 29 tab ids unchanged — zero parity churn), monolith decompositions behavior-frozen (ComplianceTab 2,531→81-line `?sub=` hub over `tabs/compliance/`; RunsTab 2,110→327; RegistryTab 1,561→342; shared `PanelScaffold`), big-data UI layer (`useQuery`/`usePaginatedQuery` + ADR-0199 'Showing N of ~M' truncation affordance + DataTable infinite scroll), serve hardening: `/api/tv5/*` auth hole closed (HTTP token + WS host/token pre-accept), Bearer-header auth, hash-then-compare token check, `/v0/replays` reads reader-gated; bounded-pagination retrofit (true keyset `/api/runs/search`, policy tail reads, lineage-edges cursor+truncated, kg-queue/admin/assets limits), ETag/304 on 8 hot polled endpoints, event-loop offload (asyncio.to_thread, sqlite lifecycle in-thread), registry schema DDL once per process. vitest harness in `web/` (55+ tests, previously zero). See CHANGELOG `[0.97.0]` + [docs/releases/v0.97.0.md](docs/releases/v0.97.0.md). New dep: lucide-react (ISC, Tier-A); removed unused `motion`. | **experimental** |
| v0.96.0 | **ADR-0220 follow-up: real per-call NATS events for `nova kg ingest --source nats`** — `capture/orchestrator.py` re-emits each locally-captured model/tool call as a `ModelCallCompleted`/`ModelCallFailed`/`ToolCallCompleted`/`ToolCallFailed` spool event (no `*Started` variant — the source data has one record per completed/failed call). New `spool_sink.emit_call_events_from_capsule()`. Also fixed a second producer/consumer field-shape mismatch: `KGIngestionPipeline.ingest_event()` now reads a real envelope's nested `payload` for `model_id`/`tool_name`/`url`. Verified end-to-end (`tests/kg/test_kg.py::test_real_producer_to_kg_pipeline_end_to_end`) — real producer, real spool envelopes, real pipeline, real KuzuDB store. Event Envelope v1 enum widened additively (4 more values), `.sha256` repinned. See CHANGELOG `[0.96.0]` and [ADR-0220](docs/decisions.md). Zero new required deps. | **experimental** |
| v0.95.0 | **ADR-0220 accepted + implemented (Option A)** — resolves the v0.94.0 NATS event-taxonomy gap: `capture/orchestrator.py`'s real event producer now emits canonical `RunStarted`/`RunCompleted`/`RunFailed` (was `run.start`/`capsule.finalize`) and threads `parent_run_id` through, so `nova lineage consume` derives real `SPAWNED_BY` edges from real captured runs — verified by a new producer-to-consumer end-to-end test. The investigation found the original "Go vs. Python taxonomy" framing was itself wrong: the Go collector's `EventType` constants were dead code; the real mismatch was within Python. Event Envelope v1 schema widened additively (4 new enum values, `.sha256` repinned); `EndpointRouted` added to the canonical `CapsuleEventType` enum. Also: PAR-ADR-002 resolved (BDFL kept the shipped 86400s/24h `pending_parent_timeout` default over the spec's 300s — see the private `design/governance/acceptance-record.md`); a missed `pyproject.toml` version bump from v0.94.0's release commit was caught and fixed. See CHANGELOG `[0.95.0]` and [ADR-0220](docs/decisions.md). Zero new required deps. | **experimental** |
...
| v0.25.0 | **C-tier compliance documentation sprint** — research-backed audit of all C-tier compliance/standards gaps across 6 regulatory domains (GDPR, HIPAA/FDA, RFC 3161, RO-Crate/PROV-JSON, Sigstore, 10-year AI governance forecast). OQ-021 resolved: `schemas/lineage-edge.schema.json` updated to Phase 3 four-type edge vocabulary (contains/spawned/delegated_to/replayed_from). 9 new compliance ADRs (ADR-0069–0077): GDPR Art.17 crypto-shredding, RFC 3161 trust chain, Sigstore keyless, PQC migration, AIBOM/CycloneDX, C2PA content credentials, DID/VC identity, EU AI Act Art.12 compliance mode, multi-region log sovereignty. 12 new compliance docs in the private `design/compliance/`. | **experimental** |
...
| v0.27.1 | **Two-tier Docker stack + deploy docs** — `make dev-up` (Postgres + dashboard) and `make prod-up` (full: + ClickHouse + NATS + Kafka + PgBouncer + JanusGraph); `novafabric-` container naming; comprehensive Data layer section in the private `design/architecture/cluster-scale.md`; `NOVA_JANUSGRAPH_URL` env var. | **experimental** |
...
| v0.38.0 | **Scale-S4: Postgres Merkle log for NovaSeal** — `PostgresMerkleLog` (psycopg3); `open_merkle_log()` URI-dispatch factory; sampled `verify_consistency()` (entry re-hash only; the root pass stays O(N), ~1.9 s at 1M entries — the 200 ms criterion is **not met**); `--full` audit flag; `[seal-postgres]` extras; `NOVAFABRIC_SEAL_DB_PATH` DSN support; CLI default DB honored at invocation time. | **experimental** |
...
| V-5 | Design partner recruitment (≥ 3 **independent** sign-offs for `Adopted` status) | **blocked — 0/3 independent**. The register holds one entry, the BDFL's own self-validation, which ADR-0034 explicitly excludes from the independence requirement; the register states "zero independent sign-offs have been recorded". Not an engineering task — cannot be unblocked by code (ADR-0223 §Context). |
...
(D3 ex-post evidence is the load-bearing moat). Integrating overview (both private):
...
| **Scale-S4** | ✅ **Shipped v0.38.0** — `PostgresMerkleLog` (psycopg3); `open_merkle_log()` URI-dispatch factory; sampled `verify_consistency()` fast path; `[seal-postgres]` extra; CLI `--db` default now honors `NOVAFABRIC_SEAL_DB_PATH` at invocation time. | NovaSeal at > 1M log entries | ⚠ **Latency criterion NOT met**: `nova seal log verify` measures ~1.9 s at 1M entries, not the targeted p99 < 200 ms — the root is recomputed from every leaf (1361 ms of Python SHA-256 alone). Backend selection via `NOVAFABRIC_SEAL_DB_PATH=postgresql://...` works as specified. |

See [Storage Architecture](docs/architecture.md#production-storage-stack-polyglot-persistence) and [Dashboard Scale Characteristics](docs/dashboard.md#scale-characteristics-and-known-limits) for the full technical context.
...
> rationale live in
> the private `design/enterprise-readiness-plan-2026-07.md`;
> sign-off record in `design/governance/acceptance-record.md`. Both are private.
...
| **W9 — Multi-modal/tool-schema + standard interop + human-agent** | C7 C2PA/tool-schema (0148) · C8 A2A/AGNTCY/OCI/SCITT interop (0149) · C9 human-agent accountability (0150) | NF-161–190 | **partly experimental** — **C7 complete — all 10 ship** (`nova provenance`, `nova toolschema`, `nova diff --media`, `nova capture-ui`) · C8 ships NF-171/172 (`nova a2a-card`, `nova a2a-objects`) · C9 future design. **ADRs accepted** (2026-07-15, /mska-approve) |
...
See the private `design/strategy/non-goals.md` for the full list (published summary: [What NovaFabric is not](docs/architecture.md#what-novafabric-is-not)). Key exclusions:
